...
| `linkedin-login.mjs` / `enrich-linkedin-pipeline.mjs` | LinkedIn authwalls every unauthenticated request, so `check-liveness.mjs` can't read it. `linkedin-login.mjs` saves a real session once (`.linkedin-session.json`, gitignored); `enrich-linkedin-pipeline.mjs` then batch-resolves `data/pipeline.md`'s LinkedIn leads — backfilling the `Job lead #{id}` placeholder with real company/title/location/posted-date and flagging closed postings — in one authenticated pass |
| `enrich-pipeline.mjs` | Same backfill as above for BuiltIn/FractionalJobs/Adzuna/Glassdoor pending rows — no login needed, but Adzuna and Glassdoor both block plain HTTP (curl, WebFetch, even Playwright's own lightweight `context.request` client all get a 403 challenge) so it drives a real rendered page per URL instead. Wired into the quick-dashboard's "Enrich Pending" button (`POST /api/pipeline/enrich-sources`) |
