...
function buildEarlierExperience(entries, partial) {
  ...
  if (!partial) {
    return entries.filter(Boolean).map(e => {
      ...
      // Company and role render on separate block-level lines (mirroring
      // .job-header / .job-role below), not joined on one line with a
      // separator — a PDF has no DOM, so a same-line "Role, Company" with a
      // CSS-generated comma between them still flattens to one continuous
      // text run once rendered, which is exactly what broke Workday's parser
      // before (career-ops#3287) and is why the .job entries above parse
      // correctly: they're genuinely on different lines in the rendered page.
      return `<div class="earlier-item">
    <div class="earlier-header"><span class="earlier-company">${escapeHtml(e.company || '')}</span>${meta}</div>
    <div class="earlier-role">${escapeHtml(e.role || '')}</div>${desc}
  </div>`;
    }).join('\n  ');
  }
